...
\begin{corollary}[Midpoint in $M_{\infty}$]
    \label{cor:midpoint_M_inf}
    \uses{def:set_of_constructable_points}
    \lean{Construction.midpoint_M_inf}
    \leanok
...
\begin{lemma}[Halving of an angle]
    ...
    \uses{def:set_of_constructable_points}
    \lean{Construction.halving_angle_M_inf}
    \leanok
...
\begin{proof}
    ...
    Now we can use Lemma \ref{lem:construction_radius} and Lemma \ref{lem:pos_real_root} to get that $\sqrt{z} \in M_{\infty}$. For $e^{\imath \frac{\alpha}{2}}$ we can compbine Lemma \ref{lem:angle_M_inf} and Lemma \ref{lem:construction_halving_angle}. Now we get that $\sqrt{z} \in M_{\infty}$, after Lemma \ref{lem:construction_mul}.
