...
\begin{lemma}[Parallel definded by basis]
...
\begin{proof}
    ...
    Define $l' = \{z, z-x+y\}$, then $l' \in \mathcal{L(M_{\infty})}$ and since a line is defind by two points and we moved them the same distence ($z-x$) $l'$ is parallel to $l$.\ref{lem:parallel_by_basis}
\end{proof}
%2.3.3
