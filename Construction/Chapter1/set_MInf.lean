...
lemma M_inf_in_M_I' (M : Set ℂ)(x:ℂ): x ∈ M_inf M ↔ ∀ᶠ n in Filter.atTop, x ∈ M_I M n := by
  ...
  simp; intro n hn; apply M_I_in_M_inf_elm M x n; apply hn; simp
...
lemma L_M_inf_iff_M_M (M: Set ℂ): l ∈ L (M_inf M) ↔ ∀ᶠ n in Filter.atTop, l ∈ L (M_I M n) := by
  ...
  exact M_I_in_M_inf_elm M z₁ n hz₁;constructor; exact M_I_in_M_inf_elm M z₂ n hz₂; exact Noteq
...
lemma C_M_inf_iff_M_M (M: Set ℂ): c ∈ C (M_inf M) ↔ ∀ᶠ n in Filter.atTop, c ∈ C (M_I M n) := by
  ...
  intro h; obtain ⟨n, hn⟩ := h; have h: ∃ z r₁ r₂, c = {c:=z, r:=(dist r₁ r₂)} ∧ z ∈ M_I M n ∧ r₁ ∈ M_I M n ∧ r₂ ∈ M_I M n := by apply hn; simp
  obtain ⟨z, r₁, r₂, h, hz, hr₁, hr₂⟩ := h; use z; use r₁; use r₂; rw [h]; simp; constructor;
  exact M_I_in_M_inf_elm M z n hz; constructor; exact M_I_in_M_inf_elm M r₁ n hr₁; exact M_I_in_M_inf_elm M r₂ n hr₂
...
lemma ill_M_inf (M: Set ℂ): ill (M_inf M) ⊆ M_inf M := by
  ...
  obtain ⟨n, hzn⟩ := zILL; apply M_I_in_M_inf_elm M z n.succ; unfold M_I ICL_M;
...
lemma ilc_M_inf (M: Set ℂ): ilc (M_inf M) ⊆ M_inf M := by
  ...
  obtain ⟨n, hzn⟩ := zILC; apply M_I_in_M_inf_elm M z n.succ; unfold M_I ICL_M;
  simp_all only [Filter.eventually_atTop, ge_iff_le, Set.mem_inter_iff, Set.mem_union, or_true, true_or]
...
lemma icc_M_inf (M: Set ℂ): icc (M_inf M) ⊆ M_inf M := by
  ...
  obtain ⟨n, hzn⟩ := zICC; apply M_I_in_M_inf_elm M z n.succ; unfold M_I ICL_M;
  simp_all only [Filter.eventually_atTop, ge_iff_le, Set.mem_inter_iff, Set.mem_union, or_true, true_or]
