...
lemma int_lc_in_M_inf (M : Set ℂ): ∀ c r₁ r₂ z₁ z₂:M_inf M, intersection_line_circle c r₁ r₂ z₁ z₂ ⊆ M_inf M := by
  ...
  have h'': intersection_line_circle a₁ a₂ a₃ a₄ a₅ ⊆ Z := by intro x hx; refine Set.mem_setOf.mpr ?_; refine SetCoe.exists.mpr ?_; simp; use a₁; constructor; simp; use a₂; constructor; simp; use a₃; constructor; simp; use a₄; constructor; simp; use a₅; constructor; simp; exact hx
  apply subset_trans (b:=Z); exact h''; exact h'
